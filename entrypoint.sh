...
then
    ...
    chmod 644 "$CERT_SSL_CA" "$CERT_SSL_CA_KEY" "$CERT_SSL_SERVER" "$CERT_SSL_SERVER_KEY" "$CERT_SSL_CHAIN"
else
    echo "Cert files present"
    ls -alh "$CERT_SSL_CA" "$CERT_SSL_CA_KEY" "$CERT_SSL_SERVER" "$CERT_SSL_SERVER_KEY" "$CERT_SSL_CHAIN"
fi

CMD_UWSGI="uwsgi --uid wgpt --gid wgpt --enable-threads --master -s 0.0.0.0:3031 --manage-script-name --mount /=wgpt_cd:app"
